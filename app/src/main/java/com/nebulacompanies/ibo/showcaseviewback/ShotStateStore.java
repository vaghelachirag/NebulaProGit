...
class ShotStateStore {
    ...
    boolean hasShot() {
        return isSingleShot() && context
                .getSharedPreferences(PrefUtils.prefShowcase, Context.MODE_PRIVATE)
    ...
    void storeShot() {
        if (isSingleShot()) {
            SharedPreferences internal = context.getSharedPreferences(PrefUtils.prefShowcase, Context.MODE_PRIVATE);
